...
Consider example of INPUT as <<0,0,1,1>>, MAXROUND=3, N=4,F=0. We see that here clearly there is no majority,
the model checker will show a run where progress is not achieved and all the decided values are -1.


For the particular expample (Bait progress) <ex here> We see that the invariant breaks, that is Consensus is still reached,
you get the output of decided value of all zeros.
