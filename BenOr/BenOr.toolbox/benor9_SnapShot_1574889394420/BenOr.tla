...
(*
    --algorithm BenOr
    {
        ...
        *)
            await (Cardinality(CollectP1Msgs(r))*2 > N) /\
                  (Cardinality(CollectP1Msgs(r)) = N - F)  ;
            ...
            if (Cardinality(ValueMsg(r,1)) > Cardinality(ValueMsg(r,0))){
            print("1");
            }
            else if(Cardinality(ValueMsg(r,0)) > Cardinality(ValueMsg(r,1))) {
            print("0");
            }
            else{
            print("-1");
...
P1R(self) == /\ pc[self] = "P1R"
             /\ (Cardinality(CollectP1Msgs(r[self]))*2 > N) /\
                (Cardinality(CollectP1Msgs(r[self])) = N - F)
             /\ IF Cardinality(ValueMsg(r[self],1)) > Cardinality(ValueMsg(r[self],0))
                   THEN /\ PrintT(("1"))
                   ELSE /\ IF Cardinality(ValueMsg(r[self],0)) > Cardinality(ValueMsg(r[self],1))
                              THEN /\ PrintT(("0"))
                              ELSE /\ PrintT(("-1"))
