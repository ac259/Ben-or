...
Agreement:
==========
We see that agreement is always satisfied, even when N < 5, F < 5, and F > N/2. Acthis property is satiscording to our
invariant property, we don't compare the initial value as the process has not been decided. 

=========
Progress:
=========
If we start with same preference value(INPUT), progress is satisfied.
